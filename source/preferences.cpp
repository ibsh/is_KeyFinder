...
void Preferences::load() {
  ...
  if (settings->contains("temporalWindow")) {
    core.setTemporalWindow((KeyFinder::temporal_window_t)settings->value("temporalWindow").toInt());
  }
  ...
  if (settings->contains("similarityMeasure")) {
    core.setSimilarityMeasure((KeyFinder::similarity_measure_t)settings->value("similarityMeasure").toInt());
  }
  settings->endGroup();
...
void Preferences::save() {
  ...
  settings->setValue("bandsPerSemitone", core.getBandsPerOctave() / SEMITONES);
  settings->endGroup();
